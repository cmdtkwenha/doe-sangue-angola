-- Align blood request statuses with the shared Portuguese app constants.

update public.blood_requests
set status = case
  when status in ('OPEN', 'Agendado', 'Triagem') then 'Aberto'
  ...
  when status = 'Em Correspondencia' then 'Em Correspondência'
  else status
end;
...
alter table public.blood_requests
  ...
  check (status in ('Aberto', 'Em Correspondência', 'Dador a Caminho', 'PIN Validado', 'Concluído', 'Cancelado'));
...
for update using (
  ...
  and status in ('Aberto', 'Em Correspondência', 'Dador a Caminho')
...
begin
  ...
  if req.status not in ('Aberto', 'Em Correspondência', 'Dador a Caminho') or coalesce(req.remaining_slots, 0) <= 0 then
